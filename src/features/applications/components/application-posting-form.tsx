"use client";
import {
  FieldDescription,
  FieldGroup,
  FieldLegend,
  FieldSet,
} from "@/components/ui/field";

import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import MaxWidthWrapper from "@/components/max-width-wrapper";
import { Button } from "@/components/ui/button";

import { cn } from "@/lib/utils";
import { FileText, Clock } from "lucide-react";
import { Form } from "@/components/ui/form";
import { CustomInput, CustomTextarea } from "@/components/customs";
import {
  ApplicationPostingFormDataType,
  applicationPostingSchema,
} from "@/lib/validations";
import { toast } from "sonner";
import { ApplicationPostingType } from "@/types";
import { useRouter } from "next/navigation";
import Spinner from "@/components/spinner";

import {
  createJobPosting,
  updateJobPosting,
} from "../actions/job-posting-actions";

type FormType = {
  type: "Create" | "Update";
  job?: ApplicationPostingType;
};

const ApplicationPostingForm = ({ type, job }: FormType) => {
  const router = useRouter();

  const initial = job
    ? { ...job }
    : {
        title: "",
        position: "",
        description: "",
        requirements: "",
      };

  const form = useForm<ApplicationPostingFormDataType>({
    defaultValues: initial,
    resolver: zodResolver(applicationPostingSchema),
  });

  const onSubmit = async (data: ApplicationPostingFormDataType) => {
    try {
      if (type === "Create") {
        const res = await createJobPosting(data);

        if (res?.error) {
          toast.error(`Failed to create job posting: ${res.error}`);
          return;
        }
        toast.success("Job posting has been created successfully!");
      } else {
        // Run update function
        if (!job) return;
        const id = job._id as string;
        const res = await updateJobPosting(id, data);
        if (res?.error) {
          toast.error(`Failed to update posting: ${res.error}`);
          return;
        }
        toast.success("Job posting has been updated successfully!");
        router.replace("/registrations/applications");
      }
    } catch (error) {
      console.error("posting creation error:", error);
      toast.error("An unexpected error occurred");
    }
  };

  return (
    <MaxWidthWrapper className="cols-span-1 md:col-span-4 flex flex-col justify-center size-full">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 p-y">
          <div className="mt-12 space-y-6 md:space-y-0 md:mt-0 md:flex items-center justify-between">
            <h1 className="text-2xl font-bold">
              {type === "Create" ? " NEW JOB POSTING" : "UPDATE JOB POSTING"}
            </h1>
          </div>
          <FieldGroup>
            <FieldSet>
              {/* Basic Information */}
              <FieldGroup className="bg-white rounded-2xl shadow-lg p-6 md:p-8">
                <div className="flex items-center space-x-3 mb-6">
                  <FileText className="size-6 text-blue-600" />
                  <div>
                    <FieldLegend className="font-bold text-accent-foreground">
                      Job Information
                    </FieldLegend>
                    <FieldDescription className="text-xs text-muted-foreground">
                      Job details
                    </FieldDescription>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <CustomInput
                    name="title"
                    control={form.control}
                    label="Title"
                    isRequired
                    placeholder="Enter job title"
                  />
                  <CustomInput
                    name="position"
                    control={form.control}
                    label="Position"
                    isRequired
                    placeholder="Enter position"
                  />
                </div>

                <div className="gap-6">
                  <CustomInput
                    name="description"
                    control={form.control}
                    label="Job description"
                    placeholder="Enter job description"
                  />
                </div>
              </FieldGroup>

              {/* Birth Information */}
              <FieldGroup className="bg-white rounded-2xl shadow-lg p-6 md:p-8 mt-6">
                <div className="flex items-center space-x-3 mb-6">
                  <Clock className="size-6 text-purple-600" />
                  <div>
                    <FieldLegend className="font-bold text-accent-foreground">
                      Requirements Information
                    </FieldLegend>
                    <FieldDescription className="text-xs text-muted-foreground">
                      Job requirements
                    </FieldDescription>
                  </div>
                </div>

                <div className="gap-6">
                  <CustomTextarea
                    name="requirements"
                    control={form.control}
                    label="Job requirements"
                  />
                </div>
              </FieldGroup>
            </FieldSet>

            {/* Submit Button */}
            <Button
              type="submit"
              disabled={form.formState.isSubmitting}
              className={cn(
                "w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold",
                "py-6 rounded-xl text-lg transition-all duration-300",
                "hover:scale-105 hover:shadow-lg",
                "disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none cursor-pointer",
              )}
            >
              {form.formState.isSubmitting ? (
                <div className="flex items-center space-x-2">
                  <Spinner />
                </div>
              ) : (
                <div className="flex items-center space-x-2">
                  <FileText className="w-5 h-5" />
                  <span>
                    {type === "Create" ? "CREATE POSTING" : "UPDATE POSTING"}
                  </span>
                </div>
              )}
            </Button>
          </FieldGroup>
        </form>
      </Form>
    </MaxWidthWrapper>
  );
};

export default ApplicationPostingForm;
